...
import { EntryDTO, CountyControllerService, OpenAPI, EntryFullDTO, Page } from "../../../../../../services/openapi"
...
export function CountyFunction(props: ICountyFunction) {
    ...
    const saveCountyMutation = useMutation({
        mutationFn: (entryDTO: EntryDTO) => CountyControllerService.saveCounty(props.worldId, entryDTO),
    })
    ...
    async function saveCounty(name: string, shortDescription: string): Promise<void> {
        ...
        return saveCountyMutation.mutateAsync({ name, shortDescription }).then(res => {
            ...
            queryClient.setQueryData(["councountyPageByWorldNameyPage", props.pageNumber, props.pageSize, props.worldName], (oldData: Page<EntryFullDTO>) => {
    ...
    async function editCounty(id: number, name: string, shortDescription: string): Promise<void> {
        ...
        return editCountyMutation.mutateAsync({ countyDTO: entryDTO, id: id }).then(_ => {
            queryClient.setQueryData(["countyPageByWorldName", props.pageNumber, props.pageSize, props.worldName],
    ...
    async function deleteCounty(id: number): Promise<void> {
        ...
        return deleteCountyMutation.mutateAsync(id).then(() => {
            queryClient.invalidateQueries({ queryKey: ["countyPageByWorldName", props.pageNumber, props.pageSize, props.worldName] })
        })
    }
