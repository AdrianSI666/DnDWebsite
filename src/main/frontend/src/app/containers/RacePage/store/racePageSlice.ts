...
interface IAddSubRacePayload {
    raceId: number,
    subRaceDTO: EntryDTO
}

interface IRemoveSubRacePayload {
    raceId: number,
    subRaceId: number
}
...
const racePageSlice = createSlice({
    ...
    reducers: {
        ...
        addNewSubRaceToRace(state, action: PayloadAction<IAddSubRacePayload>) {
            state.page.data?.at(
                state.page.data.findIndex((race) => race.race?.id === action.payload.raceId))!
                .subRaces?.push(action.payload.subRaceDTO);
        },
        removeSubRaceFromRace(state, action: PayloadAction<IRemoveSubRacePayload>) {
            let subRaces = state.page.data!.find((race) => race.race?.id === action.payload.raceId)?.subRaces?.filter((subRace) => subRace.id !== action.payload.subRaceId);
...
export const { setRacePage, fillRaceData, addRace, removeRace, updateRace, addImageToRace, removeImageFromRace, addNewSubRaceToRace, removeSubRaceFromRace } = racePageSlice.actions;
