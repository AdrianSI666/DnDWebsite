...
public class ContinentImageService implements IContinentImageService {
    ...
    public ImageDTO saveImageToContinent(MultipartFile file, Long id) {
        Continent continent = continentRepository.findById(id).orElseThrow(() -> new NotFoundException(String.format(ContinentService.CONTINENT_NOT_FOUND_MSG, id)));
        return imageService.saveImageToEntry(file, continent);
    }

    @Override
    public void deleteImageFromContinent(Long raceId, Long imageId) {
        Continent continent = continentRepository.findById(raceId).orElseThrow(() -> new NotFoundException(String.format(ContinentService.CONTINENT_NOT_FOUND_MSG, raceId)));
