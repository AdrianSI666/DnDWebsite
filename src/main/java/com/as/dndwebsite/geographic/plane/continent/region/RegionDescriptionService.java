...
public class RegionDescriptionService implements IDescriptionEntryService {
    ...
    public DescriptionDTO saveDescriptionToEntry(DescriptionDTO descriptionDTO, Long id) {
        Region region = regionRepository.findById(id).orElseThrow(() -> new NotFoundException(String.format(RegionService.REGION_NOT_FOUND_MSG, id)));
    ...
    public void deleteDescriptionFromEntry(Long entryId, Long descriptionId) {
        Region region = regionRepository.findById(entryId).orElseThrow(() -> new NotFoundException(String.format(RegionService.REGION_NOT_FOUND_MSG, entryId)));
